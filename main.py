...
class Ball:
    ...
    def __init__(self, colour):
        self.colour = colour
        self.xSpeed = 8
        self.ySpeed = self.angle_calculator(22)

    def angle_calculator(self, angle):
        C = 90 - angle
...
def main(genomes, config):
    ...
    for _, g in genomes:
        ...
        if rightSide == True:
            playerList.append(Player(1165, colours[colourCounter]))
            rightSide = False
            balls.append(Ball(colours[colourCounter]))
            colourCounter += 1
        elif rightSide == False:
            playerList.append(Player(20, colours[colourCounter]))
            rightSide = True
        if len(playerList) >= 2:
    ...
    while running:
        ...
        screenY = 5
        while screenY < screen.get_height():
            pygame.draw.rect(screen, (255, 255, 255), pygame.Rect(595, screenY, 10, 10))
            screenY += 20
